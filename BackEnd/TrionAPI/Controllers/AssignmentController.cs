...
namespace TrionAPI.Controllers
{
    ...
    public class AssignmentController : ControllerBase
    {
        ...
        [HttpGet]
        [Route("AssignmentListALL")]
        public async Task<IActionResult> AssignmentListALL(
         [FromQuery] string Opcode,
         [FromQuery] int? ClientID,
         [FromQuery] int? AssignemntID,
         [FromQuery] int page = 1,
         [FromQuery] int pageSize = 10,
         [FromQuery] int draw = 1,
         [FromQuery] string search = "",
          ...
          [FromQuery] string? ELNO = "")
        {
            ...
            using (SqlConnection con = new SqlConnection(connectionString))
            {
                using (SqlCommand cmd = new SqlCommand("SP_DT_AssignmentDataMaster", con))
                {
                    ...
                    cmd.Parameters.AddWithValue("@Opcode", Opcode);
                    cmd.Parameters.AddWithValue("@ClientID", ClientID);
                    cmd.Parameters.AddWithValue("@AssignmentID", AssignemntID);
                    cmd.Parameters.AddWithValue("@StartDate", DateTime.Now.ToString("dd-MMM-yyyy"));
                    cmd.Parameters.AddWithValue("@EndDate", DateTime.Now.ToString("dd-MMM-yyyy"));
                    cmd.Parameters.AddWithValue("@PeriodType", "");
                    cmd.Parameters.AddWithValue("@Mode", "");
                    ...
                    cmd.Parameters.AddWithValue("@SortDir", sortDirection);
                    cmd.Parameters.AddWithValue("@Search", search);
                    cmd.Parameters.AddWithValue("@ELNO", ELNO);
                    try
                    {
                        con.Open();
                        using (SqlDataReader reader = await cmd.ExecuteReaderAsync())
        ...
        [HttpGet]
        [Route("ELListALL")]
        public async Task<IActionResult> ELListALL(
          [FromQuery] string Opcode,
          [FromQuery] int? ClientID,
          [FromQuery] int page = 1,
          [FromQuery] int pageSize = 10,
          ...
          [FromQuery] string? QNNO = "")
        {
            ...
            using (SqlConnection con = new SqlConnection(connectionString))
            {
                using (SqlCommand cmd = new SqlCommand("SP_DT_AssignmentDataMaster", con))
                {
                    ...
                    cmd.Parameters.AddWithValue("@Search", search);
                    cmd.Parameters.AddWithValue("@ELNO", ELNO);
                    cmd.Parameters.AddWithValue("@QNNO", QNNO);
        ...
        [HttpDelete]
        [Route("DeleteEL/{id}")]
        public async Task<IActionResult> DeleteEL(int id)
        {
            if (id <= 0)
            {
                return BadRequest(new { result = false, message = "Invalid ID" });
        ...
        [HttpPost]
        [Route("ConfirmEL/{id}")]
        public async Task<IActionResult> ConfirmEL(int id)
        {
            if (id <= 0)
            {
                return BadRequest(new { result = false, message = "Invalid ID" });
            ...
            using (SqlConnection con = new SqlConnection(connectionString))
            {
                using (SqlCommand cmd = new SqlCommand("SP_DT_AssignmentDataMaster", con))
                {
                    ...
                    try
                    {
                        ...
                        if (rowsAffected > 0)
                        {
                            return Ok(new { result = true, message = "Assignment confirmed successfully" });
                        }
                        else
                        {
                            return NotFound(new { result = false, message = "Assignment not found" });
